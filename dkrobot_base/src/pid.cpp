...
namespace dkrobot_base
{
    ...
    void PID::updateDynamicReconfig()
    {
        ...
        // Get starting values
        dkrobot_base::ParametersConfig config;
        config.f = f_;
    ...
    void PID::updateDynamicReconfig(dkrobot_base::ParametersConfig config)
    {
        // Make sure dynamic reconfigure is initialized
        if(!dynamic_reconfig_initialized_)
            return;
    ...
    void PID::dynamicReconfigCallback(dkrobot_base::ParametersConfig &config, uint32_t /*level*/)
    {
